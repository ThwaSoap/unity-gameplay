...
{
    public class UDBStorageView : EditorWindow
    {
        [MenuItem("GamePlayTools/UDBTools/UDBStorageView")]
        static void Init() 
        {
            var window = EditorWindow.GetWindow<UDBStorageView>(false,"UDBStorageView");
            window.Show(); 
        }
        ...
        UDBStorage Storage = UDBStorage.CreateInstance();
        List<UDBPropertyTitle> Titles;
        DBContent[] Contents;
        ...
        {
            PathRoot = EditorPrefs.GetString("UDBStorageView.PageRoot","");
        }

        private void OnDisable()
        { 
            Storage.Clear();
            EditorPrefs.SetString("UDBStorageView.PageRoot", PathRoot);
        }
        ...
        void OnDrawContent(int _id) 
        {
            ...
            string key = Select.targetInstaneId.ToString();
            ...
            {
                SetContents(Storage.Select(key,SearchContent));
                IsSearchView = true;
            }
            if (GUILayout.Button("Clear")) 
            {
                ...
                {
                    IsSearchView = false;
                    SetContents(Storage.Select(key, ""));
                }
            }
            GUILayout.EndHorizontal();
        }

        void SetContents(DBContent[] _contents) 
        {
            Contents = _contents;
            PageIndex = 0;
        ...
        void DrawPageInfo() 
        {
            ...
            GUILayout.Space(25);
            EditorGUILayout.HelpBox((PageIndex + 1) + "/" + PageCount, MessageType.None);
            GUILayout.Space(25);
            if (GUILayout.Button("->"))
            {
        ...
        void DrawRow(int _begin,int _end) 
        {
            ...
            for (int i = _begin; i < _end; i++)
            {
                ...
                {
                    GUILayout.Button(v[n].String, GUILayout.MaxWidth(250),GUILayout.MinWidth(50));
                }
                GUILayout.EndHorizontal();
            }
            GUILayout.EndScrollView();
